package com.example.glassstream.stream
...
/**
 * On-device HEVC decode for the live preview (ported from Meta's CameraAccess sample).
 *
 * Decodes the compressed HEVC frames delivered by the DAT SDK and renders them directly to a
 * [Surface] via MediaCodec (the GPU handles YUV->RGB). Replicates the SDK's VideoDecoder
 * NAL-parsing / enqueue logic: parse NAL units, cache config, activate on the first keyframe,
 * then feed each NAL separately.
 */
class HevcDecoder {
    ...
    companion object {
        ...
        // Hardware HEVC decoders that corrupt this stream into fragments/trash; the SDK's own
        // decoder skips them too. Prefer a software decoder instead (see createHevcDecoder).
    ...
    // preview black after the decoder deactivates on a full queue.
